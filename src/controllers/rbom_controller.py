...
class RBOMMAC:
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        ...
        # agent_outputs, qi, q_infer, role_outputs = self.forward(ep_batch, t_ep, test_mode=test_mode, t_env=t_env)
        agent_outputs, _ = self.forward(ep_batch, t_ep, test_mode=test_mode, t_env=t_env)
        ...
        role_avail_actions_own = th.gather(self.role_action_spaces.unsqueeze(0).repeat(self.n_agents, 1, 1), dim=1, index=self.selected_roles_own.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, self.n_actions).long()).squeeze()
        role_avail_actions_own = role_avail_actions_own.int().view(ep_batch.batch_size, self.n_agents, -1)

        chosen_actions = self.action_selector.select_action(agent_outputs[bs], avail_actions[bs], role_avail_actions_own[bs], t_env, test_mode=test_mode)
        return chosen_actions, self.selected_roles, self.selected_roles_own, role_avail_actions_own
    
    def get_roles_own(self, selected_roles):
        selected_roles = selected_roles.reshape(-1, self.n_agents, self.n_agents)
        roles_own = th.zeros(selected_roles.shape[0], self.n_agents).to(self.args.device)
        for i in range(self.n_agents):
            roles_own[:,i] = selected_roles[:, i, i]
        return roles_own.view(-1)

    def forward(self, ep_batch, t, test_mode=False, t_env=None, roles_own=None, roles=None):
        agent_inputs = self._build_inputs(ep_batch, t)

        # infer all agents roles
        self.role_hidden_states = self.role_agent(agent_inputs, self.role_hidden_states)       # bs*n, h_dim
        role_outputs = None
        # =============================================================== ori_learner ==============================================================================
        if t % self.role_interval == 0:
            role_outputs_own, role_outputs, visiable_infer = self.role_selector(self.role_hidden_states, self.role_latent)    ## bs*n, n_role   bs*n*n, n_role  bs*n*n,2
            if roles is None:
                self.selected_roles = self.role_selector.select_role(role_outputs, test_mode=test_mode, t_env=t_env).squeeze()   # [bs * n_agents * n_agents]
                self.selected_roles_own = self.role_selector.select_role(role_outputs_own, test_mode=test_mode, t_env=t_env).squeeze()      # [bs * n_agents]
            else:
                self.selected_roles_own = roles_own
                self.selected_roles = roles

            role_outputs = role_outputs.reshape(ep_batch.batch_size, self.args.n_agents, self.args.n_agents, -1)
            role_outputs_own = role_outputs_own.reshape(ep_batch.batch_size, self.args.n_agents, -1)
    
        # infer others ac using roles
        self.hidden_states = self.agent(agent_inputs, self.hidden_states)    # bs*n, h_dim

        # agent_outs, qi, q_infer = self.agent.decision_module(self.hidden_states, self.selected_roles, self.selected_roles_own, self.role_action_spaces)    ## bs, n, n_ac
        # return agent_outs, qi, q_infer, ([None, None] if role_outputs is None else [role_outputs, role_outputs_own])

        agent_outs = self.agent.decision_module(self.hidden_states, self.selected_roles, self.selected_roles_own, self.role_action_spaces)
        return agent_outs, ([None, None] if role_outputs is None else [role_outputs, role_outputs_own])
    ...
    def update_role_action_spaces(self):
        ...
        # th.save(action_repr,"ac_repr.pt")
