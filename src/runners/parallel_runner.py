...
class ParallelRunner:
    ...
    def run(self, test_mode=False):
        ...
        while True:
            ...
            if self.args.agent == 'mat':
                actions_log, actions, v_loc = self.mac.select_actions(self.batch, t_ep=self.t, t_env=self.t_env, bs=envs_not_terminated, test_mode=test_mode)
                cpu_actions = actions.to("cpu").numpy()

                actions_chosen = {"actions": actions.unsqueeze(1), "actions_log": actions_log, "values_pred": v_loc }
                ...
                actions_chosen = { "actions": actions.unsqueeze(1),  "value": value, "entropy": entropy, "log_probs": log_probs }
            else:
