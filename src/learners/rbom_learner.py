...
class RBOMLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        roles_shape_o = batch["roles_own"][:, :-1].shape
        ...
        roles[:, :roles_shape_o[1]] = batch["roles_own"][:, :-1]
        roles_own = batch["roles_own"]
        roles_all = batch["roles_all"]
        ...
        for t in range(batch.max_seq_length):
            # agent_outs, qi_outs, qij_outs, out_list = self.mac.forward(batch, t=t, roles_own=roles_own[:,t,:,:].reshape(-1), roles=roles_all[:,t,:,:].reshape(-1))
            agent_outs, out_list = self.mac.forward(batch, t=t, roles_own=roles_own[:,t,:,:].reshape(-1), roles=roles_all[:,t,:,:].reshape(-1))
            role_outputs_all, role_outputs_own = out_list[0], out_list[1]
            mac_out.append(agent_outs)
            # qi_out.append(qi_outs)
            # qij_out.append(qij_outs)
            if t % self.role_interval == 0 and t < batch.max_seq_length - 1:
                role_out.append(role_outputs_own)
                role_out_all.append(role_outputs_all)
        mac_out = th.stack(mac_out, dim=1)  # Concat over time
        # qi_out = th.stack(qi_out, dim=1)  # Concat over time
        # qij_out = th.stack(qij_out, dim=1)  # Concat over time
        role_out = th.stack(role_out, dim=1)  # Concat over time
        role_out_all = th.stack(role_out_all, dim=1)  # Concat over time
        ...
        for t in range(batch.max_seq_length):
            # target_agent_outs, _, _, out_list = self.target_mac.forward(batch, t=t, roles_own=roles_own[:,t,:,:].reshape(-1), roles=roles_all[:,t,:,:].reshape(-1))
            target_agent_outs, out_list = self.target_mac.forward(batch, t=t, roles_own=roles_own[:,t,:,:].reshape(-1), roles=roles_all[:,t,:,:].reshape(-1))
            target_role_outs = out_list[1]
        ...
        role_infer_target = role_out.unsqueeze(2).repeat(1,1,self.args.n_agents,1,1)   ## bs,t,n,n,n_role
        role_infer_loss = ((role_out_all - role_infer_target.detach())**2).sum()/role_mask[:,:,:,None,None].repeat(1,1,self.n_agents,self.n_agents,self.args.n_roles).sum()
        # qij_target = qi_out.unsqueeze(2).repeat(1,1,self.args.n_agents,1,1)  ## bs, t, n, n, n_ac
        # q_infer_loss = ((qij_out.reshape(qij_target.shape[0], qij_target.shape[1], self.n_agents,self.n_agents,self.args.n_actions) - qij_target.detach())**2).sum() \
        # /role_mask[:,:,:,None,None].repeat(1,1,self.n_agents,self.n_agents,self.args.n_actions).sum()

        loss += role_loss
        loss += role_infer_loss
        # loss += q_infer_loss
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            self.logger.log_stat("loss", (loss - role_loss - role_infer_loss).item(), t_env)
            self.logger.log_stat("role_loss", role_loss.item(), t_env)
            self.logger.log_stat("role_infer_loss", role_infer_loss.item(), t_env)
