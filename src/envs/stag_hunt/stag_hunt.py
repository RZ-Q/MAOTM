...
class StagHunt(MultiAgentEnv):
    ...
    def get_visibility_matrix(self):
        # self.agents:  n, bs, 2  position in grid
        ...
        for b in range(self.batch_size):
            for a in range(self.n_agents):
                if self.agents_not_frozen[a,b]:
                    agents_pos = self.agents[[a], b,:]
                    al_ids = [ al_id for al_id in range(self.n_agents) if al_id > a]
                    for _, al_id in enumerate(al_ids):
                        target_pos = self.agents[al_id, b,:]
                        if self._is_visible(agents_pos, target_pos):
                            arr[a, al_id] = arr[al_id, a] = 1
        return arr
